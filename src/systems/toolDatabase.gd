...
func _ready() -> void:
	load_items_from_json("res://data/tools.json")
...
func load_items_from_json(file_path: String) -> void:
	...
	for item_data in raw_data:
		...
		var expected_path = "res://assets/tools/" + clean_file_name + ".png"
		...
		if ResourceLoader.exists(expected_path):
			...
			# default to ho image if the png image cannot be found
			new_item.item_texture = load("res://assets/tools/hoe.png")
			print("failed to find path '", expected_path, "' for item: '", new_item.item_name, "'")
			
		items[item_data["name"]] = new_item


func get_tools_in_category(category: String) -> Dictionary:
